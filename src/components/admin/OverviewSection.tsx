...
export default function OverviewSection({ overview }: OverviewSectionProps) {
  const stats = [
    ...
    { label: 'Joined', value: overview?.joinedCount || 0, icon: UserCheck, color: 'text-brand-orange' },
  ...
  const currentStatus = overview?.event?.status || 'setup';
  const status = statusLabels[currentStatus] || statusLabels.setup;
  const joinedLetters = overview?.joinedLetters || [];
      ...
      {joinedLetters.length > 0 && (
        <div className="card mt-4">
          ...
          <div className="flex flex-wrap gap-2">
            {joinedLetters.map((letter: string, idx: number) => (
              <div
                key={idx}
                className="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-50 text-orange-600 font-bold text-lg border border-orange-200 shadow-xs"
              >
                {letter}
              </div>
            ))}
